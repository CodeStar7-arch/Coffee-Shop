...
export default function Input({ className, label, error, id, ...props }) {
    return(
        ...
        id={id}
        className={cn(
            "h-11 w-full rounded-xl border bg-white/[0.05] px-4 text-[var(--cream)] placeholder:text-white/25 outline-none backdrop-blur-sm transition-all duration-200",
            "border-white/12 focus:border-[var(--amber)]/50 focus:bg-white/[0.08] focus:ring-[var(--amber)]/15",
